...
\begin{table}[htb]
    ...
    \begin{tabular}{c|c|c}
        Metric & $\hat{R}$ & ESS \\
        \hline
        $\mathcal{O}_{norm}$ & 1.003 & 69,540 \\
        $\mathcal{O}_{deg}$ & 1.004 & 69,500 \\
        $\mathcal{O}_{det}$ & 1.005 & 69,319 \\
        $\mathcal{O}_{var}$ & 1.006 & 69,131 \\
        $\mathcal{O}_{gen}$ & 1.009 & 68,769
    \end{tabular}
...
Much of the data appears to have a power law distribution, to confirm this we compute $x_\min$ according to Clauset, Shalizi, and Newman, fit the power with MLE and plot on a log-log histogram. This is seen in figure~\ref{fig:mcmc-score-dist-log}

\begin{figure}[htb]
    \centering
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{thesis/figures/agg_score_alex_deg_hist.png}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{thesis/figures/agg_score_alex_det_hist.png}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{thesis/figures/agg_score_alex_norm_hist.png}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{thesis/figures/agg_score_num_gen_hist.png}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{thesis/figures/agg_score_edge_var_hist.png}
...
\begin{figure}[htb]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{thesis/figures/agg_score_edge_var_log_distribution.pdf}
        \caption{$\mathcal{O}_{var}$}
        \label{fig:mcmc-score-dist-log-var}
    \end{subfigure}
    
    \caption{Log-log histogram of the score for each objective function under MCMC samples at $\gamma=1/10$. The red line indicates the fitted power law distribution, fitted for $x>x_\min$}
    \label{fig:mcmc-score-dist-log}
\end{figure}

\subsection{Direct Ascent}
We perform direct accent starting from the seed triangulation of .... This seed triangulation has ... tetrahedra, we performed this direct accent multiple times at different temperatures, and compared the maximum achieved score at each level, an example of this is in figure.... In all cases we found that a lower temperature lead to better results, this suggests that the graph is connected enough that greedy ascent is sufficient (that is, the neighbour with the greatest score is accepted at each step). We perform greedy ascent for each objective function for ... steps, the chains are presented in figure .... We see that for .... the ascent profile is rather deterministic, suggesting ... However, for ... we observe a more complex objective function geometry.

\subsection{Simulated Annealing}
We perform simulated annealing as described in .... This is done for $10,000$ iterations with a target acceptance rate of $20\%$, this acceptance rate was chosen heuristically based on a number of different smaller experiments. The simulated annealing took ... to run. Initial results showed a ``runaway'' in the size of the triangulations, so the potential was modified to become $-\infty$ if the number of tetrahedra exceeded $30$, thereby being a more fair comparison to the direct accent. The chains for each objective function are plotted in ...

\subsection{Comparison of Classical Optimisation}
The best result from each optimisation technique is compiled in table ..., this table also includes the number of standard deviations away from the mean as estimated by the MCMC samples. We observe that ... achieves the best score, suggesting .... For the MCMC samples, we perform a ... test for normality and fine a ..., under this assumption of normality, we can estimate the percentile of each maximum obtained by each method, and estimate the expected number of samples required to obtain such a score if random sampling was done. While this normality assumption is weak, and it is likely invalid to extrapolate the distribution so far from the MCMC samples, it provides an estimate of the increased efficiency of the search algorithms.
